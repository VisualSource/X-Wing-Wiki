...
const loader = async ({ params }: LoaderFunctionArgs) => {
  ...
  const [factionId, shipType, pilotId, loadoutId, ..._] =
    params.shipid.split(":");

  return defer({
    data: new Promise<object>(async (ok, reject) => {
      const [pilotData, upgradeData] = await Promise.all([
        fetch(`/std_loadouts/${factionId}.json`).then((value) =>
          value.json(),
        ) as Promise<
          [
            string,
            {
              ...
              pilots: {
                id: string;
                standardLoadout?: Record<string, string[]>;
                xws: string;
              }[];
            },
          ][]
        >,
        fetch(`/upgrades.json`).then((value) => value.json()) as Promise<
          Record<string, { xws: string; sides: { ability?: string }[] }[]>
        >,
      ]);
      ...
      const pilot = data.pilots.find(
        (pilot) => pilot.xws === pilotId && pilot.id === loadoutId,
      );
      ...
      if (pilot?.standardLoadout) {
        upgrades = Object.entries(pilot.standardLoadout).reduce(
          (acc, [key, items]) => {
            const updateType = upgradeData[key];
            if (!updateType) return acc;

            for (const item of items) {
              const fullupgrade = updateType.find(
                (value) => value.xws === item,
              );
        ...
        upgrades.sort(
          (a, b) =>
            a.sides.reduce((acc, c) => (c?.ability?.length ?? 0) + acc, 0) -
            b.sides.reduce((acc, c) => (c?.ability?.length ?? 0) + acc, 0),
        );
      }
