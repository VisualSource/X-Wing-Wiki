...
<Calculate /> action, it gains one calculate token. A ship is **CALCULATING**
while it has at least one calculate token. Calculate tokens are circular, green
tokens. A calculating ship follows these rules:
