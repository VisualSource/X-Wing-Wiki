<Header title="ship" />A ship is composed of a plastic miniature, base, pegs, a ship
token, and ID tokens.

- A ship's plastic miniature must match the ship's type as indicated on the
  ship card.
- A ship must use the dial that matches the [ship's type](/article/Ship_Type).
- Some plastic miniatures extend beyond their plastic base. For this reason,
  ...
  overlap obstacles and hang over the edge of the [play area](/article/Play_Area) without issue.
