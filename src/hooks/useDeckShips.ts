...
const useDeckShips = () => {
    const [data, setData] = useLocalStorage<{ ship: Ships.ShipSearchData, idx: number }[]>("deck", [], {
    ...
    return {
        ...
        removeShip(id: string, idx: number) {
            setData((current) => {
                if (!current) return current;
                return current.filter(item => item.ship.id !== id && item.idx !== idx).map((value, i) => ({ ship: value.ship, idx: i }));
            });
        }
    }
}
